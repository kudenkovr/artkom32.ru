...
class ModelCatalogCategory extends Model {
	public function addCategory($data, $id=null) {
		...
		if (!empty($id)) {
			$this->db->query("UPDATE " . DB_PREFIX . "category SET category_id = '" . (int)$id . "' WHERE category_id = '" . (int)$category_id . "'");
			$category_id = $id;
		}
	...
	// For b2b API
	public function getAllCategoriesParents() {
		$cache_name = 'api.category.parents';
		$data = $this->cache->get($cache_name);
		
		if (empty($data)) {
			$sql = 'SELECT cp.category_id as id, cp.path_id AS parent
						FROM '.DB_PREFIX.'category_path AS cp
						LEFT JOIN '.DB_PREFIX.'category AS c USING(category_id)
						WHERE c.status=1
						ORDER BY cp.path_id';
			$categories = $this->db->query($sql)->rows;
			$data = array();
